...
const Modal = ({ setTokenStyle, closeModal }: { setTokenStyle: any, isModalShow: boolean, closeModal: any }) => {
    const tokens = [
        ...
        return (<>
                ...
                <div className="absolute shadow-lg rounded-md flex flex-col -translate-x-64 translate-y-60 bg-[#7e4bf7] justify-center w-[342px] z-10 pt-1">
                    <div className="flex justify-between px-4 py-2">
                        <article>Select a currency</article>
                        <button onClick={closeModal}>&times;</button>
                    </div>
                    {tokens.map((token) => (
                        <button key={token.title} onClick={() => { setTokenStyle(token.title);}} className="flex flex-col border-t-[1px] border-[#808086] py-2 px-4">
